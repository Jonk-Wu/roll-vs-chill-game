...
import Gevendra from "../assets/20251116162133_259_7.jpg";
import { BsInfoCircle, BsMenuButton } from "react-icons/bs";
import { AiFillGithub, AiFillInstagram } from "react-icons/ai";
import { GrLinkedinOption } from "react-icons/gr";
import RollVsChillGame from "../RollVsChillGame.jsx";

const Card = () => {
  const [ShowGame, setShowGame] = useState(false);
  if (ShowGame) {
    return <RollVsChillGame />;
  }
  return (
    <div className="card relative w-[290px] rounded-t-2xl rounded-b-[11rem] pt-[2.5rem] pb-[3.5rem] text-center shadow-shadow ">
      <div className="w-[124px] h-[124px] bg-firstlight rounded-full mx-auto mb-5 mt-0 flex justify-center items-center">
        <button
          ...
          className="w-[100px] h-[100px] bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full text-white font-bold hover:scale-105 transition-transform"
        >
          开始游戏
        </button>
      </div>
      <h3 className="text-white text-[1.25rem] font-semibold mb-1 ">
        Jonk Wu
      </h3>
      <p className="text-white text-[.75rem]  font-medium ">
        College Student
      </p>
      ...
      <div className="absolute top-[-1px] left-0 w-full bg-white pt-[2.5rem] pb-[3.5rem] rounded-t-2xl rounded-b-[11rem] info group">
        ...
        <div className="grid gap-y-1 mb-6 transition-transform -translate-x-16 duration-500 group-hover:translate-x-0 ">
          <h3 className=" text-title text-[1.25rem] font-semibold">
            Jonk Wu
          </h3>
          <p className="text-[0.75rem]">Student at Wenzhou Medical University</p>
          <p className="text-[0.75rem]">Zhejiang, China</p>
        </div>

        <div className="flex justify-center gap-x-4 transition-transform -translate-x-20 duration-[1.2s] group-hover:translate-x-0 ">
          <a
            className="w-[32px] h-[32px] bg-firstlight rounded-full grid place-items-center transition-transform duration-200 hover:-translate-y-1"
            target="_blank"
            href="https://www.linkedin.com/in/jonk-wu-125456338/"
          >
            <span className="inline-flex card p-1 rounded-full text-white text-base ">
              <GrLinkedinOption />{" "}
            </span>
          </a>
          ...
          <a
            className="w-[32px] h-[32px] bg-firstlight rounded-full grid place-items-center transition-transform duration-200 hover:-translate-y-1"
            target="_blank"
            href="https://www.instagram.com/wujonk/"
          >
            <span className="inline-flex card p-1 rounded-full text-white text-base ">
              <AiFillInstagram />
            </span>
